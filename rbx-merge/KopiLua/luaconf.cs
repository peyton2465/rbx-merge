...
//#endif
		...
		*/
		...
		public static Stream getStreamFromResourceFile(Assembly assembly, string filename)
		{
			filename = Path.GetFileName(filename);
			var resource = assembly.GetManifestResourceNames().FirstOrDefault(x => x.EndsWith(filename));
			if (resource == null || resource.Equals(default)) return null;
			...
			for (int i=0; mode[i] != '\0'; i++)
				switch (mode[i])
				{
					case 'r': 
						...
						var filestream = getStreamFromResourceFile(Assembly.GetEntryAssembly(), str);
						if (filestream != null) return filestream;
						filestream = getStreamFromResourceFile(Assembly.GetExecutingAssembly(), str);
						if (filestream != null) return filestream;
